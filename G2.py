...
bad_model = "models/anonymous_bd_net.h5"
...
# np.save('activations/Activations_bd2.npy', avg_activations)

'''The code below sorts the array according to the activations of each channel'''

avg_activations_channels = np.zeros((1,80))
clean = np.load('activations/Activations_bd2.npy')
for i in range(0, 80):
    avg_activations_channels[0][i] = np.mean(clean[:,:,i])
id_sort = np.argsort(avg_activations_channels, axis=-1)

'''#Pruning'''
'''To check how pruning affects the accuracy, we have created a loop which prunes the network based on the sorted activations array. So, first, the lest activated channel get prunes. 
...
# np.save('accuracies/bd2/clean_accuracies_bd2.npy', clean_accuracies)

#Plot the accuracy vs no of channels pruned graph.
clean_accuracies_loaded = np.load('accuracies/bd2/clean_accuracies_bd2.npy')
